...
The Rowhammer Tester design also exposes a LiteX UARTBone bridge on Channel D at 1 Mbaud.
This provides a local control path when Ethernet is unavailable. Assuming the four channels
are `/dev/ttyUSB0` through `/dev/ttyUSB3`, start a LiteX server with:

```sh
litex_server --uart --uart-port /dev/ttyUSB3 --uart-baudrate 1000000
```

Keep the server running while using the Rowhammer Tester scripts in another terminal.
